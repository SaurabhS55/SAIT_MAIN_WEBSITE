...
function Home() {
  useEffect(() => {}, []);
  const text1 = "Beyond The Limitations Next To The Perfection";
          ...
          <div className="grid-item-1">
            ...
            {/* <h1 className="overflow-hidden leading-6 text-white main-heading1 text-3xl md:text-4xl lg:text-5xl font-bold mb-4">
              {Array(text1).map((char, index) => (
                <span
                  className={`main-heading2 animate-text-reveal inline-block [animation-fill-mode:backwards] ${
                    index >= 11 && index <= 21 ? " text-pink-400" : " "
                  }`}
                  key={`${char}-${index}`}
                  style={{ animationDelay: `${index * 0.05}s` }}
                >
