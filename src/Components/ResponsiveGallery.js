...
const ResponsiveGallery = forwardRef((props, ref) => {
  ...
  const recentImages = images.slice(0, 13);

  return (
    <div>
      {recentImages.length !== 0 ? (
        <div className="w-11/12 mx-auto mt-20 p-6" ref={ref}>
          <h2 className="colorfont text-3xl mb-6">Recent Gallery</h2>
          <div className="responsive-gallery grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {recentImages.map((image) => (
              <div key={image._id} className="relative group overflow-hidden">
                <img
                  src={image.imageLink}
                  alt="Memory"
